...
var analytic, assert, createIndex, css, ctype, deriveStructure, deriveStructureWithValues, fs, generateFromHtml, html, iterator, logging, markers, mode, model, output, respond, sentenceSplitter, timer, titleAndAbstract, util, verbex;
...
deriveStructure = function(elements) {
  ...
  return console.dir(maxStructure);
};

deriveStructureWithValues = function(elements, variationLimit) {
  var element, iterate, maxStructure, _i, _len;
...
generateFromHtml = function(req, name, res, docLogger, callback) {
  ...
  for (_p = 0, _len7 = extremes.length; _p < _len7; _p++) {
    ...
    for (physicalPageSide = _r = 0; _r <= 1; physicalPageSide = ++_r) {
      ...
      if (!(repeatSequence > 0)) {
        console.log('no repeat ' + extreme.goalName + ' ' + 'detected in article' + ' ' + 'in pass' + ' for ' + (physicalPageSide === 0 ? 'even pages' : 'odd pages'));
      } else {
        console.log(repeatSequence + ' ' + 'repeat' + ' ' + extreme.goalName + 's' + ' ' + 'detected in article' + ' ' + 'in pass' + ' ' + (physicalPageSide === 0 ? 'even pages' : 'odd pages'));
      }
    }
  }
  ...
  */
  ...
  groups = [];
  group = [];
  for (t = _ae = 0, _len14 = tokens.length; _ae < _len14; t = ++_ae) {
    token = tokens[t];
    if (token.metaType === 'regular') {
      connect_token_group({
        group: group,
  ...
  for (_af = 0, _len15 = groups.length; _af < _len15; _af++) {
    group = groups[_af];
    sentence = '';
  ...
  markSentence = function(sentenceIdx) {
    var marker, matchedMarkers, _ah, _ai, _len17, _len18;
    sentence = groups[sentenceIdx];
    matchedMarkers = [];
    if (sentence != null) {
      for (_ah = 0, _len17 = sentence.length; _ah < _len17; _ah++) {
        token = sentence[_ah];
        if (token.metaType !== 'delimiter') {
          for (_ai = 0, _len18 = docSieve.length; _ai < _len18; _ai++) {
            marker = docSieve[_ai];
  ...
  markSentence(0);
  for (_ah = 0, _len17 = tokens.length; _ah < _len17; _ah++) {
    token = tokens[_ah];
    if (token.page == null) {
...
exports.go = function(req, name, res, docLogger) {
  ...
  return generateFromHtml(req, name, res, docLogger, respond);
};

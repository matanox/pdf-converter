...
var Promise, connect, connection, dataMap, diffsTable, docTablesDefinition, error, exec, getSqlSingleResult, init, knex, logging, maxDbStrLength, purge, reinit, reqCounter, runIDsTableDef, runsTableDef, successCount, tableDefs, write;
...
getSqlSingleResult = function(resultArray) {
  var resultObj;
  if (resultArray.length === 1) {
    resultObj = resultArray[0];
    return resultObj[Object.keys(resultObj)[0]];
  } else {
    throw "query intended to result in one item resulted in " + resultArray.length;
  }
};

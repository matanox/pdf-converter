...
var Promise, connect, connection, dataMap, diffsTable, docTablesDefinition, error, exec, gradingTable, init, knex, logging, maxDbStrLength, pdfTablesDefinition, purge, reinit, reqCounter, runIDsTableDef, runsTableDef, successCount, tableDefs, write;
...
pdfTablesDefinition.forEach(function(table) {
  return table.type = 'pdf';
});
...
tableDefs.forEach(function(table) {
  var _ref;
  if ((_ref = table.type) === 'docTable' || _ref === 'pdf') {
...
dataMap = function(context, dataType, data) {
  var key, mapping;
  if (dataType === 'pdfFonts' || dataType === 'pdfMeta' || dataType === 'pdfToHtml') {
    mapping = {
      output: data,

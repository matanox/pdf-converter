...
var directory, fs, host, http, logging, nconf, port, rdbms, util;
...
exports.go = function(res) {
  var aggregateRequestsWait, batchRunID, filename, flood, i, makeRequest, maxFiles, parallelism, requests, responses, toRequest, _i, _j, _k, _len, _len1, _ref;
  if (typeof appInvoked !== "undefined" && appInvoked !== null) {
  ...
  if (toRequest.length > 0) {
    ...
    return res.end("Starting to process all files...");
  } else {
    return res.end('No files to process in directory, doing nothing.');
  }
};

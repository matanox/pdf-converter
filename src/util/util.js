...
var anySpaceChar, clone, contains, dataWriter, endsWith, fs, isAnyOf, logging, os, replaceAll, startsWith, timelog, trimLast, winston,
  _this = this;
...
exports.mkdirRecursive = function(path) {
  return require('mkdirp').mkdirp(path, function(err) {
    if (err) {
      throw err;
    }
  });
...
exports.replaceAll = replaceAll = function(string, from, to) {
...
exports.terminalClickableFileLink = function(string) {
  return "file://" + (process.cwd()) + "/" + (replaceAll(string, ' ', '%20'));
};

os = require('os');

exports.simpleGenerateRunID = function() {
  var hostname, now, timeISO;

...
var dataWriter, expected, expectedSequence, getContinuouslyStyled, hasNext, isTitlishCase, isTitlishCaseSequence, logging, noHeadersDocs, optionallyCasedInTitle, sameness, separateness, startsWithDigit, styleContinuity, test, testNext, tripleIterator, xml,
...
expectedSequence = function(text) {
  var e, _i, _len;
...
module.exports = function(context, tokens) {
  var anyFound, detectionComments, getLevelStyle, h, level1Style, matchedExpectation, prev, regularTokens, seqEnd, sequenceAsText, t, token, _i, _j, _len;
  ...
  for (t = _i = 0, _len = tokens.length; _i < _len; t = ++_i) {
    ...
    if (token.paragraphOpener) {
      ...
      if (!(matchedExpectation || ((level1Style != null) && sameness(token, level1Style)))) {
        continue;
      }
      detectionComments = [];
